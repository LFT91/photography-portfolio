...
export function GalleryEditor({
  ...
  const source = items ?? localPhotos;
  ...
  const [selection, setSelection] = useState<{
    ...
    room: lockedCategory ?? categories[0],
    editing: false,
    keys: [],
  });
  ...
  const selectedKeys =
    selection.room === room && selection.editing === editing
      ? selection.keys
      : [];

  const setSelectedKeys = (keys: string[] | ((prev: string[]) => string[])) => {
    setSelection((prev) => {
      const current =
        prev.room === room && prev.editing === editing ? prev.keys : [];
      return {
        room,
        editing,
        keys: typeof keys === "function" ? keys(current) : keys,
      };
    });
  };
